...
class RF(nn.Module):
    def __init__(self, num_classes=2, max_depth=3, min_samples_split=30, min_samples_leaf=3):
        super().__init__()
    ...
    def fit(self, train_loader, optimizer, device, dtype):
        logging.warning("RF is now only implemented for non-image data, modals[0] will be ommited")
